...
(function () {
  ...
  accordion.forEach((item) => {
    ...
    ulList.dataset.height = ulList.offsetHeight;
    let list = item.querySelectorAll('li');
    Array.from(list)
      .slice(5)
      .forEach((v) => (v.style.display = 'none'));
    ulList.dataset.closeHeight = ulList.offsetHeight;

    let moreHeight = ulList.dataset.height - ulList.offsetHeight;
    let moreBtn = item.querySelector('.btnMoreLess');
    let moreCheck = false;
    if (list.length > 5) {
      moreBtn.addEventListener(
        ...
        function (e) {
          if (!moreCheck) {
            item.style.height = `${Number(item.dataset.itemHeight) + Number(moreHeight)}px`;
            list.forEach((v) => (v.style.display = 'block'));
            ulList.style.height = `${ulList.dataset.height}px`;
            moreCheck = true;
            e.target.innerHTML = 'less';
          } else {
            item.style.height = `${item.dataset.itemHeight}px`;
            ulList.style.height = `${Number(ulList.dataset.closeHeight) - 2}px`;
      ...
      let sibLi = item.parentElement.querySelectorAll('.accordionList');
      sibLi.forEach((v) => {
